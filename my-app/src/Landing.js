...
import { doc, getDoc, updateDoc, collection, addDoc, getDocs, deleteDoc, arrayUnion, arrayRemove } from "firebase/firestore";
import { useNavigate } from "react-router-dom";
import "./Landing.css";

const GENRES = ["Pop", "Rock", "Jazz", "HipHop", "Classical", "Electronic", "Country", "Reggae", "RnB", "Metal"];

const fetchSongsByGenre = async (genre, excludeIds = []) => {
  try {
    const response = await fetch(
      `https://itunes.apple.com/search?term=${encodeURIComponent(genre)}&media=music&limit=50`
    );
    const data = await response.json();
    const filtered = data.results.filter(track => !excludeIds.includes(track.trackId));
    return filtered.slice(0, 3).map(track => ({
      id: track.trackId,
...
export default function Landing() {
  ...
  const [shownSongIds, setShownSongIds] = useState({});
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          setName(`${data.firstName || ""} ${data.lastName || ""}`.trim() || user.email);
          if (!data.preferences || data.preferences.length < 3) setShowPreferences(true);
  ...
  const fetchAllSongs = async () => {
    ...
    let allSongs = [];
    const newShownIds = { ...shownSongIds };
    ...
    setTimeout(() => {
      setSongs(allSongs);
      allSongs.forEach((_, i) => setTimeout(() => setShownCards(prev => [...prev, i]), i * 150));
    }, 50);

    setShownSongIds(newShownIds);
  };
  useEffect(() => { fetchAllSongs(); }, [userPreferences]);
  ...
  const handleSavePreferences = async () => {
    if (selectedGenres.length < 3) { setError("Select at least 3 genres."); return; }
    try {
  ...
  const handleConfirmAddToPlaylist = async (playlistId) => {
    ...
    try {
      if (playlistId === "new") {
        ...
        const playlistRef = collection(db, "users", user.uid, "playlists");
        await addDoc(playlistRef, { name: newPlaylistNameModal.trim(), songs: [songToAdd] });
      } else {
  ...
  const handleRemoveSongFromPlaylist = async (playlistId, song) => {
    await updateDoc(doc(db, "users", user.uid, "playlists", playlistId), { songs: arrayRemove(song) });
    fetchUserPlaylists();
  };
      ...
      {showPreferences ? (
          ...
          <div className="genre-buttons">
            {GENRES.map(g => (
              <button key={g} onClick={() => handleGenreToggle(g)}
                className={selectedGenres.includes(g) ? "selected" : ""}>{g}</button>
            ))}
          </div>
        ...
        <>
          {userPreferences.length > 0 && (
            <>
              <p>You like {userPreferences.map((g,i) => i===userPreferences.length-1? `'${g}'` : `'${g}', `)}. Here are some suggestions:</p>
              <button className="refresh-btn" onClick={fetchAllSongs}>Refresh Suggestions</button>

              <div className="songs-container">
                {songs.map((song, index) => (
                  <div key={song.id} className={`song-card ${shownCards.includes(index) ? "show" : ""}`}>
                    <div className="song-card-clickable" onClick={() => playSong(song)}>
              ...
              {showAddToPlaylistModal && (
                <div className="modal-overlay">
                  <div className="modal-content">
                    <h3>Select a playlist to add "{songToAdd?.title}"</h3>
                    <div className="existing-playlists">
                      {playlists.map(p => (
                        <button key={p.id} onClick={() => handleConfirmAddToPlaylist(p.id)}>{p.name}</button>
                      ))}
                    </div>
                    <div className="new-playlist">
                      ...
                      <button onClick={() => handleConfirmAddToPlaylist("new")}>Create & Add</button>
                    </div>
                    <button className="close-modal" onClick={() => setShowAddToPlaylistModal(false)}>❌</button>
                  </div>
                </div>
              )}
